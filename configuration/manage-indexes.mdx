...
Click **Create index** on the **Settings → Indexes** page to define a new Quickwit index from a form — index ID, schema mode, field mappings, timestamp field, retention, and more — with no YAML or direct Quickwit calls. Rootprint creates the index in Quickwit and opens its detail page when you save. The same form is available over the API as `POST /api/indexes`.
