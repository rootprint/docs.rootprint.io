...
The **Settings → Indexes** section gives admins a single place to create indexes, inspect every index Quickwit knows about, configure how each is presented in the search UI, manage the data sources that feed it, and delete indexes that are no longer needed.
...
Click **Create index** on the **Settings → Indexes** page to define a new Quickwit index from a form, with no YAML or direct Quickwit calls. The form covers:

- The **index ID** (starts with a letter, 3–255 characters of letters, digits, `-`, or `_`).
- The schema **mode** (`dynamic`, `lenient`, or `strict`), which decides what Quickwit does with the fields you didn't declare.
- The **field mappings**: each field's name, type, and per-type options such as tokenizer and the `fast` and `stored` flags.
- The **timestamp field**, picked from the datetime fields you defined.
- An optional **retention policy**, **storage URI**, **tag fields**, and **commit timeout**.

Rootprint creates the index in Quickwit and opens its detail page when you save. The same form is available over the API as `POST /api/indexes`.

Some schemas need a type or setting the form doesn't expose: `bytes`, `array<T>`, `object`, or `dynamic_mapping` tuning. For those, write the index definition in YAML and send it to Quickwit directly. See [Create a custom index](/configuration/custom-indexes) for the full schema reference.
...
Data sources are the pipelines Quickwit uses to receive or pull documents for the index, for example the built-in `_ingest-api-source` that backs the HTTP and OTLP ingest endpoints, or an external source that pulls from Amazon Kinesis, Apache Kafka, Apache Pulsar, or files announced over SQS. Each row shows the source ID, its type, and whether it is enabled. Click a row to open the source detail page, or use **Create source** to add a new one.
...
Click **Create source** on the Sources tab. Rootprint can create four source types; pick one and Quickwit starts pulling documents into this index as soon as the source is saved.
...
| **Apache Pulsar**                      | One or more Pulsar topics.                                                                             |
...
<Tab title="Apache Pulsar">

| Field             | Required | Notes                                                                                |
| ----------------- | -------- | ------------------------------------------------------------------------------------ |
| **Topics**        | Yes      | One or more Pulsar topics, one per line (e.g. `persistent://public/default/my-topic`). |
| **Address**       | Yes      | The Pulsar broker URL (e.g. `pulsar://localhost:6650`).                              |
| **Consumer name** | No       | The consumer name Quickwit registers with Pulsar. Defaults to `quickwit`.            |

</Tab>
</Tabs>

All four source types also accept three optional fields:
...
Open any Rootprint-created source — Kinesis, file, Kafka, or Pulsar — from the list to edit it in place. The same Connection and Transform tabs appear, pre-filled with the current configuration; the source ID and type are read-only. Saving applies the change to Quickwit immediately. Sources created directly in Quickwit and built-in sources are shown read-only.
...
- Per-user preferences, saved views, shared links, and search audit records tied to the index.
