...
description: 'Inspect, configure, and delete indexes from Rootprint: the index list, per-index field-role mapping, fields, sources, and deletion.'
---
...
| **Trace ID field**  | `trace_id`                         | Dot-notation path to the trace ID inside a log document. Links logs to traces in both directions: a **Trace** tab in the log detail drawer, and log counts on spans. See [Read a trace](/traces/explore#correlate-logs-with-traces). |
...
| Field                  | Notes                                                                                                                                                            |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Input format**       | The document format on the wire. Defaults to `json`. Other choices: `plain_text`, `otlp_logs_json`, `otlp_logs_protobuf`, `otlp_traces_json`, `otlp_traces_protobuf`. |
