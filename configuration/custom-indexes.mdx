...
description: 'Define a Quickwit index with your own schema, tokenizers, and retention policy, then point Rootprint at it.'
---

The bundled `otel-logs-v0_9` index works for any application that emits OpenTelemetry logs. For everything else (audit trails, third-party log formats, application events with a specialized schema), create an index in Quickwit with the fields and types your data needs, then map it into the Rootprint UI from the index's [Configuration tab](/configuration/manage-indexes#configuration-tab).

This page covers the index definition you send to Quickwit. For routing logs once the index exists, see the [HTTP endpoint](/send-logs/http).
...
The index appears in **Settings → Indexes** on the next page load. No Rootprint restart needed.

<Note>
	Rootprint proxies index management actions to Quickwit but does not currently expose an "create
	index" form in the admin UI. Create the index through Quickwit's REST API or CLI, then configure
	its display from Rootprint.
</Note>
...
- Enforce search bounds: the Rootprint time picker passes `start_timestamp` and `end_timestamp` based on this field.
...
1. Open **Settings → Indexes** and click the new index.
2. On the **Configuration** tab, set the [field mappings](/configuration/manage-indexes#field-role-mappings) so the message, level, and traceback render correctly.
3. Create an [ingest API key](/api/overview) scoped to the index and route writes through the [HTTP endpoint](/send-logs/http) or, for OpenTelemetry-shaped data, the [OTLP endpoint](/send-logs/otlp).
...
- [Manage indexes](/configuration/manage-indexes#field-role-mappings): Rootprint field-role mapping.
