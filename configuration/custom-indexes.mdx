...
description: 'Define a Quickwit index with your own schema, tokenizers, and retention from the Rootprint Create-index form: no YAML required.'
---
...
Starts with a letter; 3–255 characters of letters, digits, `-`, or `_`. This is the ID Quickwit stores the index under and the value you pass as the `index` when ingesting.
...
Pick the `datetime` field Quickwit treats as the index's time axis, chosen from the datetime fields you define below. Quickwit uses it to:
...
Add a row per field with **Add field**, then set its name, type, and options. At least one field is required, and the toggles come preselected to Quickwit's defaults (`indexed`, `stored`, and `fast` on).
...
| **Tag fields**           | Field names (one per line) whose values are indexed as split tags, letting Quickwit prune splits before scanning. Best for low-cardinality fields. |
