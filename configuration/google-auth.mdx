...
<Steps>
  ...
  <Step title="Enter credentials in Rootprint">
    In Rootprint, go to **Administration → Authentication** and find the **Google Auth** section. Enter:
  ...
  <Step title="Apply the configuration">
    Restart the Rootprint container to apply the new configuration:
...
To disable Google sign-in, go to **Administration → Authentication → Google Auth** and click **Remove Google Auth**, then restart the server:

```bash
docker compose restart rootprint
```

After the restart, the Google sign-in button is removed from the sign-in page.
...
<Warning>
	...
	**Administration → Users** and invite them again — the new invite link prompts them to set a
	password.
</Warning>
