...
| `SEARCH_AUDIT_RETENTION_DAYS` | `30`    | How many days of search activity audit rows to keep. Rows older than the window are pruned on the same hourly tick that captures index stats. `30` is also the minimum; a lower value fails startup. |
