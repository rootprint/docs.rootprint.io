...
| `SEARCH_AUDIT_RETENTION_DAYS` | `30`    | How many days of search activity audit rows to keep. Rows older than the window are pruned on the same hourly tick that captures index stats. `30` is also the minimum — a lower value fails startup, because the activity dashboard's longest range is 30 days and a shorter window would leave it incomplete. |
...
<Warning>
	A wrong `TRACE_INDEX_ID` fails silently. Rootprint treats a missing span store as an empty trace
	so that a logs-only deployment does not error, which means a typo looks like "no spans" rather
