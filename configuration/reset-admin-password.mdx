...
<Warning>
	...
	account and use **Settings → Users → Reset password** instead — it sends the locked-out user a new
	invite link to set a fresh password.
</Warning>
...
Run the two statements against the bundled `db` service. Deleting a `user` row cascades to that
user's sessions, linked accounts, API keys, saved queries, and preferences via foreign keys.

```bash
# 1. Remove every admin user (cascades to their sessions, accounts, and keys).
