...
| Monitoring API         | Service health panels computed from server spans: request rate, errors, latency, and top endpoints |
...
| `429`  | Rate limit exceeded — Rootprint's own (`TOO_MANY_REQUESTS`) or Quickwit's, passed through (`UPSTREAM_RATE_LIMIT`). Honour `Retry-After`. |
