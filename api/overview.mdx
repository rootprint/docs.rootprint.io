---
title: 'API reference'
description: 'Use the Rootprint HTTP API to ingest logs, search logs from external tools, and automate administration against your own Rootprint deployment.'
---
...
<CardGroup cols={3}>
  ...
  <Card title="Search API" icon="search" href="/api/reference/search-logs/search-logs-with-a-search-api-key">
...
| ---------------------------------------- | -------------------------- | ----------------------------------- |
| Send logs over NDJSON or OTLP            | Ingest API key (bearer)    | **Administration → API keys**       |
| Search logs from an external tool        | Search API key (bearer)    | **Administration → API keys**       |
...
<Note>
  ...
  contract is visible, but automation should prefer token-based ingest and search where possible.
</Note>
