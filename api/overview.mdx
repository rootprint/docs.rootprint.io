...
| Service health API     | Service health computed from spans: request rate, errors, latency, top endpoints, downstream calls, and the failing spans themselves |
...
| Auth settings API      | Google, GitHub, and OpenID Connect credentials, allow-lists   |
| System monitoring API  | Health, cluster overview, activity, and metrics               |

## Timestamps

Every timestamp the API takes is **Unix epoch seconds** — `startTs` and `endTs` on search, histogram, field-value, field-discovery, export, monitoring, and stats requests, and `startTime`/`endTime` in a share or saved-view body. Log documents keep their own nanosecond fields; that is document data, not a request parameter.
