...
The **Services** page (at `/monitoring`) turns the spans in your span store into the request-level view of a deployment: how much traffic each service takes, how much of it fails, how slow it is, which endpoints account for the time, what each service calls downstream, and which individual spans failed.
...
Every view opens with the same five figures for the current service and time range:
...
| **p95 latency** | p95 over the selected service's requests. With no service selected it reads **Slowest p95** — the worst service's p95, named beside the figure. |
...
| **Services**     | All services  | Every service ranked by request volume, with failure share and p50/p95. Selecting a row scopes the page to it. |
...
Only **SERVER** spans (`span_kind:2`) are counted as requests — one span per inbound request. Client, internal, and producer/consumer spans are ignored there, so a request crossing three services counts once per service rather than once per hop. A request is an error when its span status is `error`.
...
The **Dependencies** tab groups a service's outbound spans — client (`span_kind:3`) and producer (`span_kind:4`) — by span name, and ranks them by total time, with call count, p50, and p95. Under each call name it lists up to three peers, read from `server.address`, so an HTTP client span shows which hosts it talked to.
