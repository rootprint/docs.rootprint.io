...
services:
  ...
  rootprint:
    image: ghcr.io/oleksandr-zhyhalo/rootprint:latest
    restart: unless-stopped
    depends_on:
      db:
...
<Note>Rootprint only supports Quickwit `0.9` and later.</Note>
...
```bash
curl -o docker-compose.yml https://raw.githubusercontent.com/rootprint/docs.rootprint.io/main/docs/files/docker-compose.standalone.yml
```
...
Customize the deployment by setting environment variables under the `environment` key of the `rootprint` service. See the [environment variables reference](/configuration/environment-variables) for the full list, including rate limits and authentication settings.
