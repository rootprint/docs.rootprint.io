...
| Google Cloud Storage       | `gs://bucket/path`                        | Service account JSON via `GOOGLE_APPLICATION_CREDENTIALS`                                     |
...
S3-compatible providers need a `flavor` set on the storage config so Quickwit picks the right path-style and multipart settings. Google Cloud Storage can alternatively be reached through the S3-compatible interface using the `gcs` flavor with HMAC keys. See the [Quickwit storage reference](https://quickwit.io/docs/configuration/storage-config) for the YAML structure and the full env-var list.
