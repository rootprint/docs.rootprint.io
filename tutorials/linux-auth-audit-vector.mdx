...
description: 'Ship /var/log/auth.log to Rootprint with Vector and search SSH, sudo, and PAM events by user, source IP, and severity.'
---

`/var/log/auth.log` carries every authentication event a Debian or Ubuntu host produces: SSH attempts, `sudo` invocations, `su`, PAM, `login`. This tutorial sets up Vector as a `systemd` service that tails the file, parses each line into structured fields (timestamp, severity, source IP, user), and ships the result to Rootprint over OTLP. Records land in `otel-logs-v0_9` and become searchable by program, source address, target user, and severity.
...
<Steps>
  <Step title="Pick the target index and create an ingest API key">
    Logs land in whichever index your ingest token is scoped to. This tutorial uses the built-in
...
<Step title="Install Vector">
  ...
  <Step title="Write the Vector config">
    ...
    URL and `<your-ingest-token>` with the API key you copied in step 1.
...
Two `remap` transforms run in sequence. The first, `parse_auth`, turns each raw syslog line into structured fields. The second, `to_otlp`, packs those fields into the OTLP wire format that Rootprint's ingest endpoint expects.
...
`to_otlp` assembles the OTLP envelope. `service.name` is derived from the parsed `program` so SSH events show as `service_name:sshd`, sudo as `service_name:sudo`, and so on. Rootprint's service filter will list every auth program separately. Per-event attributes are pushed conditionally inside `if exists(...)` guards, so the OTLP record never carries empty `stringValue` attributes for SSH-only fields on a `sudo` event.

## Useful searches

Once Vector has been running for a minute or two against an active host, the parsed fields make these queries possible. Run them in the Rootprint search box against `otel-logs-v0_9`.
...
- **Records arrive but `attributes.process.name` is missing**: the syslog regex didn't match. Most often this means the line is in RFC 5424 format or your daemon emits a non-standard prefix. Inspect the raw `body.message` in Rootprint to see the shape of the line, then adjust the regex in the `parse_auth` transform.
