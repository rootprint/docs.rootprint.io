...
For non-OpenTelemetry data — audit trails, custom application events, third-party log formats — create an index with the schema your data needs. See [Create a custom index](/configuration/custom-indexes) for the YAML structure, field types, tokenizers, and retention policy. Once it exists, configure how Rootprint maps its fields from the index's [Configuration tab](/configuration/manage-indexes#configuration-tab).
