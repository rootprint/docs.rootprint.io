...
<Tip>
	Pasting a bare 32-character hex trace ID into the search box opens that trace directly instead of
	running a search. Wrap it in quotes to search for it as text instead. Use the `trace_id:` field
	query above when you want the **logs** for a trace rather than its spans. See
