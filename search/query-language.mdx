...
The Rootprint search box accepts the [Quickwit query language](https://quickwit.io/docs/reference/query-language). Queries combine field-scoped clauses with boolean operators, and the same syntax works whether you type it in the UI or pass it to the search API.
...
<Tip>
	...
	**Settings → Indexes → Fields**. The same value indexed with `default` and `raw` produces
	different match behavior.
</Tip>

## Time ranges

The Rootprint UI exposes a time picker that bounds every query to a time window — you don't write the timestamp clause yourself. If you call the search API directly, pass `start_timestamp` and `end_timestamp` as **seconds** (regardless of the field's precision):
