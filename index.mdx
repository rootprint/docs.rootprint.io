...
<CardGroup cols={2}>
  <Card title="Live demo" icon="play" href="https://demo.rootprint.io">
    Read-only logs and traces from a running demo cluster. No signup, nothing to install.
  </Card>
  ...
  <Card title="Traces" icon="git-branch" href="/traces/overview">
    Ingest OpenTelemetry spans over OTLP and pair them with the logs they produced.
  </Card>
</CardGroup>
...
- **Log ingestion**: push logs over OTLP or the NDJSON HTTP endpoint
- **Trace correlation**: ingest OpenTelemetry spans over OTLP and link them to the logs they produced
- **Service health**: request rate, error rate, latency, downstream calls, and failing spans per service and endpoint, derived from your spans
...
- **User management**: invite-based access control with optional Google, GitHub, or OpenID Connect sign-in
