  ...
  <Tab title="Vector">
    ...
    <Steps>
      ...
      <Step title="Write the Vector config">
        ...
        host and `<your-ingest-token>` with the API key you copied above.
...
- **`body` looks like raw JSON instead of the message alone**: the access log is using a non-default `format` directive in the Caddyfile. The parser expects Caddy's default JSON encoder. Either remove the `format` directive or extend the config to handle the alternate shape.
