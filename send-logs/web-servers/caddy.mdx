...
<Steps>
  <Step title="Pick the target index and create an ingest API key">
    ...
    schema. In **Administration → API keys**, click **Create API key**, give it a name, pick
...
<Steps>
  ...
  <Step title="Save the Vector config">
    ...
    your Rootprint base URL and `<your-ingest-token>` with the token you copied in step 1.
...
- **`401`, `403`, `413`, `415` from Rootprint** — same response codes as a misconfigured Vector setup of any kind. See [Send logs with Vector](/send-logs/log-agents/vector#troubleshooting) for full diagnoses.
