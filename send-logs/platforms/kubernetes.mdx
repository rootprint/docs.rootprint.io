...
<Steps>
  ...
  <Step title="Create values.yaml">
    ...
    - **Override the receiver as `filelog`, not `file_log`.** The preset uses the old key and the chart renames it afterwards; a `file_log` block is ignored. Helm prints a `[DEPRECATION]` notice for it.
...
`DEBUG` is not inferred: `\bdebug\b` false-positives too often against arbitrary pod output. Emit debug records through an OpenTelemetry SDK that sets `severityNumber` itself.
...
- **`403` from Rootprint**: the API key is not scoped to `otel-logs-v0_9`. Create a new key scoped to that index.
