...
<Warning>
	Mounting `/var/run/docker.sock` into a container grants it the ability to enumerate and inspect
	every container on the host. This is standard for log collectors that use the Docker API, but
	worth being explicit about. The mount below is read-only (`:ro`).
</Warning>
