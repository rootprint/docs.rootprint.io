...
This page ships container logs from a Linux host to Rootprint over OTLP. **Vector** (default tab) reads the Docker daemon socket: it auto-discovers containers and attaches `container.name`, `container.image.name`, and friends with no extra wiring. The **OpenTelemetry Collector** reads container log files off disk; it works without the socket but cannot resolve friendly container names the same way; see the caveats in its tab. Vector is the recommended path for Docker.
