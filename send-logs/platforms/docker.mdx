...
description: 'Ship every container on a Linux host to Rootprint with a Vector sidecar that reads from the Docker daemon and emits OTLP.'
---
...
`service.name` is derived from the container's name (with the leading `/` Docker prefixes stripped), so a query for `service_name:my-api` filters Rootprint to one container's events. `host.name` comes from the Vector container's hostname.
