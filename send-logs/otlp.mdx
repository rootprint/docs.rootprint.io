...
| `503`  | Upstream Quickwit error or network failure, or the request ran past `INGEST_PROXY_TIMEOUT_MS` (120 seconds by default). `Retry-After: 5`. |

On a `413`, lower the agent's batch size (`batch.max_bytes` in Vector, `send_batch_max_size` on the Collector's `batch` processor) or raise the proxy's body limit.
