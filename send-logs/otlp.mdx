...
Rootprint maps OTLP errors to gRPC-style status codes. Upstream `4xx` statuses pass through as sent, and any upstream `5xx` or network failure is clamped to a retryable `503`.

| Status | Meaning                                                                 |
| ------ | ----------------------------------------------------------------------- |
| `200`  | Accepted. Empty body when every record was accepted, or carrying `partial_success` when Quickwit rejected some. |
| `400`  | Empty request body, or Quickwit rejected the payload.                   |
| `401`  | Missing Bearer API key.                                                 |
| `403`  | Invalid or unknown ingest API key.                                      |
| `413`  | Payload too large. Usually a reverse proxy, but an upstream `413` passes through too.  |
...
A `413` usually means a reverse proxy in front of Rootprint rejected an oversized request body, though an upstream `413` from Quickwit now passes through unchanged as well. Lower the agent's batch size (`batch.max_bytes` in Vector, `send_batch_max_size` on the Collector's `batch` processor) or raise the proxy's body limit.
...
<Note>
	Rootprint also accepts OTLP **traces** at `POST /v1/traces`, with the same ingest key and the
	same protobuf-only rule. Spans ignore the index scope described above: they always go to the span
	store named by `TRACE_INDEX_ID`, whatever index the key is pinned to. See
