...
Rootprint exposes an OTLP HTTP endpoint that accepts OpenTelemetry logs, authenticates them with an ingest API key, and forwards them to Quickwit's OTLP endpoint. The target index is fixed by the API key.
...
- `Content-Encoding`: Rootprint decompresses the body before indexing. Supported values: `gzip` / `x-gzip`, `zstd`, `deflate` / `x-deflate`, and `identity` (or no header) for uncompressed. Any other value is rejected. OTEL SDKs send `gzip` or no compression.
