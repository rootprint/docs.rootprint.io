...
The target index is pinned by the ingest API key, not by the SDK. To write to a different index, create an ingest API key scoped to that index in **Administration → API keys** and use its value in the `Authorization` header. See [Manage indexes](/configuration/manage-indexes).
