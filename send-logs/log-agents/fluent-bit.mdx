...
description: 'Tail log files on a Linux host with Fluent Bit and ship them to Rootprint over OTLP.'
---

[Fluent Bit](https://fluentbit.io) is a lightweight log forwarder built for edge and host-level collection. This page sets up Fluent Bit as a `systemd` service on a Linux host: it tails files under `/var/log/`, batches the records, and ships them to Rootprint's OTLP endpoint with a Bearer token. Records land in your `otel-logs-v0_9` index, the same index every OpenTelemetry SDK writes to.
...
- **`413` from Rootprint** — a single batch exceeded 10 MB on the wire. Lower the output batch size
