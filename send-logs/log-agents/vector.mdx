...
- **Requests hang and Vector retries the same batch**: Vector's `request.timeout_secs` (60 by
  default) is shorter than Rootprint's ingest timeout (120 seconds by default), so Vector gives up
  while Rootprint is still waiting on Quickwit. Set `INGEST_PROXY_TIMEOUT_MS` below Vector's
  timeout so Vector receives a retryable `503` instead, and send smaller batches.
