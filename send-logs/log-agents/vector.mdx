...
<Step title="Install Vector">
  ...
  <Step title="Write the Vector config">
    ...
    Rootprint's 10 MB OTLP body limit even after gzip variance.
...
- **`403` from Rootprint** — the token's index scope does not match `otel-logs-v0_9`, or the token
  was revoked. Mint a fresh ingest API key for `otel-logs-v0_9` in **Administration → API keys**.
- **`413` from Rootprint** — a single batch exceeded 10 MB on the wire. Lower
