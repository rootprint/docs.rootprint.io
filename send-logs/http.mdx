...
<Step title="Create an ingest API key">
	In **Administration → API keys**, click **Create API key**, pick the target index, and choose the
...
- **403** — the API key is invalid or revoked. Create a new ingest API key for that index in **Administration → API keys**.
