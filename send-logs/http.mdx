...
description: 'Authenticated HTTP endpoint in front of Quickwit: push logs to the index attached to your ingest API key using whatever schema that index expects.'
---

Rootprint exposes an authenticated HTTP endpoint at `/api/ingest/ndjson`. Each ingest API key is scoped to a single index; Rootprint forwards the request body unmodified to Quickwit. Use this endpoint when you already control the wire format and want to write to a custom-schema index.
...
<Steps>
  <Step title="Pick the target index">
    Decide which Quickwit index you'll write to. The built-in OTEL index is `otel-logs-v0_9`: see [Indexes](/indexes) for its schema, or follow [Create a custom index](/configuration/custom-indexes) to define your own first.
  </Step>

<Step title="Create an ingest API key">
	In **Settings → API keys**, click **Create ingest key**, give it a name, and pick the target
	index. Copy the value shown on creation. API keys are scoped to one index. You cannot
	reuse one across indexes.
</Step>

  <Step title="POST NDJSON to the endpoint">
...
Rootprint forwards the body unchanged. You must send the format your API key's Quickwit index expects: typically NDJSON with `Content-Type: application/x-ndjson` (the endpoint also accepts `application/json`). The payload must match the target index schema; Quickwit rejects documents that don't. See [Indexes](/indexes) for the built-in `otel-logs-v0_9` schema, or the index's [Configuration tab](/configuration/manage-indexes#field-role-mappings) for Rootprint's display mapping over your custom schema.
...
- **400**: NDJSON parse error. Common causes: a trailing comma between objects, or a JSON array instead of one object per line.
