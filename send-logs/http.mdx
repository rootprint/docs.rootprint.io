...
description: 'Authenticated HTTP endpoint: push logs to the index attached to your ingest API key using whatever schema that index expects.'
---

Rootprint exposes an authenticated HTTP endpoint at `/api/ingest/ndjson`. Each ingest API key is scoped to a single index. Use this endpoint when you already control the wire format and want to write to a custom-schema index.

<Note>
	For OpenTelemetry-instrumented apps, prefer the [OTLP endpoint](/send-logs/otlp). It handles
	batching, retries, and schema mapping for you.
</Note>
...
<Steps>
  <Step title="Pick the target index">
    Decide which index you'll write to. The built-in OTEL index is `otel-logs-v0_9`: see [Indexes](/indexes) for its schema, or follow [Create a custom index](/configuration/custom-indexes) to define your own first.
  </Step>
...
Rootprint forwards the body unchanged. You must send the format your API key's index expects: typically NDJSON with `Content-Type: application/x-ndjson` (the endpoint also accepts `application/json`). The payload must match the target index schema; See [Indexes](/indexes) for the built-in `otel-logs-v0_9` schema, or the index's [Configuration tab](/configuration/manage-indexes#field-role-mappings) for Rootprint's display mapping over your custom schema.
