...
description: 'Ship Node.js logs to Rootprint using the OpenTelemetry SDK, Pino, or Winston over OTLP HTTP.'
---

Rootprint accepts OpenTelemetry logs over OTLP HTTP. Node apps can use the plain OTEL SDK, or plug OTLP into [Pino](https://getpino.io/) or [Winston](https://github.com/winstonjs/winston). All three routes ship records through the same endpoint and land in the OTEL logs index pinned by your ingest API key (`otel-logs-v0_9` in these examples).
...
<Steps>
  ...
  <Step title="Set environment variables">
    ...
    <Warning>The `%20` after `Bearer` is required: OpenTelemetry SDKs expect URL-encoded values in `OTEL_EXPORTER_OTLP_LOGS_HEADERS`.</Warning>
  ...
  <Step title="Minimal working example">
    <Tabs>
      <Tab title="OTEL SDK">
        ...
        await provider.shutdown(); // optional — flushes and stops the processor cleanly
        ```
      </Tab>
      <Tab title="Pino">
        ```javascript
      ...
      <Tab title="Winston">
        ...
        // Short-lived scripts should shut down the provider before exit so the
